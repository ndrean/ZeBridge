...
* Backend Services /  micro-VM: Running native SQLite or standard PostgreSQL with their local database. For example, a micro-VM seeds (or is kept warm in sync) and can start a DuckDB analytical job against a local SQLite replica.
...
🚦 CDC events that contain unsupported columns **suspends** the table. In particular, extensions use their own type are dynamically generated by the Postgres server.
...
Postgres' schemas are directly used by PGlite. It is `PostGis` "ready".

SQlite uses the following type casting:
