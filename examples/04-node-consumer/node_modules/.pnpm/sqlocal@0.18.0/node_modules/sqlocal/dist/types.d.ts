...
export type Sqlite3StorageType = (string & {}) | 'memory' | 'opfs' | 'local' | 'session';
export type Statement = {
    sql: string;
    params: unknown[];
};
export type ReturningStatement<Result = unknown> = Statement | (IsAny<KyselyQuery<unknown>> extends true ? never : KyselyQuery<Result>) | (IsAny<DrizzleQuery<unknown, never>> extends true ? never : DrizzleQuery<Result extends SqliteRemoteResult<unknown> ? any : Result[], 'sqlite'>);
export type SqlTag = typeof sqlTag;
export type StatementInput<Result = unknown> = ReturningStatement<Result> | ((sql: SqlTag) => ReturningStatement<Result>);
...
export type ReactiveQuery<Result = unknown> = {
    ...
    subscribe: (onData: (results: Result[]) => void, onError?: (err: Error) => void) => {
        unsubscribe: () => void;
    };
};
...
export type DatabasePath = (string & {}) | ':memory:' | 'local' | ':localStorage:' | 'session' | ':sessionStorage:';
...
export type UserFunction = CallbackUserFunction | ScalarUserFunction | AggregateUserFunction | WindowUserFunction;
