...
def main() -> int:
    ...
    h = lib.zb_client_open(json.dumps({"url": zb.nats_server(), "credsPath": zb.creds_for(who), "dbPath": db, "principal": who, "clientId": "py-client-gap", "tables": ["users", TABLE]}).encode())
    if not h:
